...
async def get_hitokoto_result():
    ...
    if hitokoto_count>0:
        ...
        return reponse(data=hitokoto_result,code=200,message="success",hitokoto=hitokoto_str)
    else:
        _msg = "未读取到一言，即将重新读取"
        logger.warn(_msg)
        print(_msg)
        init_hitokoto()
        return reponse(data={'msg':_msg},code=500,message="error",hitokoto="")
